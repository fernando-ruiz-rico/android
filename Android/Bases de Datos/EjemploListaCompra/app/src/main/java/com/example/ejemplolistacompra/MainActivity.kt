...
fun ListaCompra(modifier: Modifier = Modifier) {
  ...
  val listaItems = remember { mutableStateListOf<Item>() }
  val coroutineScope = rememberCoroutineScope()

  LaunchedEffect(Unit) {
    listaItems.addAll(itemDao.getAll())
  }

  Column(modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(16.dp)) {
    FormularioCompra {
      coroutineScope.launch {
        val item = Item(descripcion = it)
        val id = itemDao.insert(item) // Insertamos en base de datos
        listaItems.add(item.copy(id = id.toInt()))
      }
    }
    HorizontalDivider()
    ListaItems(listaItems) {
      coroutineScope.launch {
        itemDao.delete(it) // Borramos en base de datos
        listaItems.remove(it)
      }
    }
  }
}
